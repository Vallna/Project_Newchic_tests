...
@pytest.fixture
def chrome_options(chrome_options): # эта фикструра определяет с какими опциями надо запустить браузер
    # chrome_options.binary_location = '/usr/bin/google-chrome-stable'
    # chrome_options.add_argument('--headless')
    chrome_options.add_argument('--no-sandbox') # эту опцию передаем чтобы браузер не блокировался
    chrome_options.add_argument('--log-level=DEBUG') #задаем уровень log для браузера
...
def pytest_runtest_makereport(item, call):
    # This function helps to detect that some test failed #эта функция нужна чтобы мы сохраняли в переменную статус тесткейса
...
@pytest.fixture
def web_browser(request, selenium): # 'эта фикстура вызывается в каждом тесткейсе и выполнится перед запуском теста
...
def get_test_case_docstring(item): #эта фиксттура помогает написать названия тестов красиво в терминале при выводе резельтатов.
    ...
    #чтобы показать эту строку документации вместо имени тестового примера в отчетах.
    ...
    if item._obj.__doc__:
        ...
        # Generate the list of parameters for parametrized test cases:  Сгенерируйте список параметров для параметризованных тестовых случаев
...
def pytest_collection_finish(session):
    ...
    """
    #Эта функция изменяла названия тестовых примеров "на лету"
